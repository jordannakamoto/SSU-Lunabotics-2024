...
address = 16   # Address of the Tic, that is its device number
...
def main():
  ...
  position = tic.get_current_position() # this gets the starting position/number of steps 
  ...
  while position != int(user_input): # Run till the steps are equal to the user's input 
    tic.exit_safe_start()
    ...
    if int(user_input) == real_position or upd_value==GPIO.HIGH : # if push-button is pressed stop the stepper motor 
      tic.haltAndHold() 
      time.sleep(1.5)
