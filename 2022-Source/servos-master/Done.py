# Uses the smbus2 library to send and receive data from a Tic.
# Works on Linux with either Python 2 or Python 3.
#
# NOTE: The Tic's control mode must be "Serial / I2C / USB".
# NOTE: For reliable operation on a Raspberry Pi, enable the i2c-gpio
#   overlay and use the I2C device it provides (usually /dev/i2c-3).
# NOTE: You might nee to change the 'SMBus(3)' line below to specify the
#   correct I2C device.
# NOTE: You might need to change the 'address = 11' line below to match
#   the device number of your Tic.



import RPi.GPIO as GPIO
import keyboard
import time 
from smbus2 import SMBus, i2c_msg

class TicI2C(object):
  def __init__(self, bus, address):
    self.bus = bus
    self.address = address
 
  # Sends the "Exit safe start" command.
  def exit_safe_start(self):
    command = [0x83]
    write = i2c_msg.write(self.address, command)
    self.bus.i2c_rdwr(write)
 
  # Sets the target position.
  #
  # For more information about what this command does, see the
  # "Set target position" command in the "Command reference" section of the
  # Tic user's guide.
  def set_target_position(self, target):
    command = [0xE0,
      target >> 0 & 0xFF,
      target >> 8 & 0xFF,
      target >> 16 & 0xFF,
      target >> 24 & 0xFF]
    write = i2c_msg.write(self.address, command)
    self.bus.i2c_rdwr(write)


  def haltAndHold(self):
    command = [0x89]
    write = i2c_msg.write(self.address, command)
    self.bus.i2c_rdwr(write)

  def energize(self):
    command = [0x85]
    write = i2c_msg.write(self.address, command)
    self.bus.i2c_rdwr(write)



  # Gets one or more variables from the Tic.
  def get_variables(self, offset, length):
    write = i2c_msg.write(self.address, [0xA1, offset])
    read = i2c_msg.read(self.address, length)
    self.bus.i2c_rdwr(write, read)
    return list(read)
 
  # Gets the "Current position" variable from the Tic.
  def get_current_position(self):
    b = self.get_variables(0x22, 4)
    position = b[0] + (b[1] << 8) + (b[2] << 16) + (b[3] << 24)
    if position >= (1 << 31):
      position -= (1 << 32)
    return position
  
def menu():
    print("Enter (f) for full opertaion, (c) for controller, (q) end program: ")    
    
    
# Open a handle to "/dev/i2c-3", representing the I2C bus.
bus = SMBus(1)
 
# Select the I2C address of the Tic (the device number).
address = 15    # Address of the Tic, that is its device number
address2 = 16  

tic = TicI2C(bus,address)
tic2 = TicI2C(bus,address2)

 # Represents /dev/i2c-3
# below is a function for pressing a key to give 10 steps 
# "w" for up & "s" for down


def main():
  
  menu()
  
  option = input('What do you want to ? ')
  while option != 'q':
    
    
    while True:
    
      if(option =='f'):
     
        tic2.energize()
        tic.energize()

        print(tic2.get_current_position())
        print(tic.get_current_position())



        des = input("Enter (h) for Homing, (d) for Dumping, (m) for Mining,(q) for main menu: ")
   
        if(des == 'h'):
          home = 2000
          position = tic2.get_current_position()
          position2 = tic.get_current_position()
      
 

          print("Current position is: {}.".format(position))
          print("Current position is: {}.".format(position2))
      
      
  
          while True:
            tic2.energize()
            tic2.exit_safe_start()
            tic2.set_target_position(int(home))
            real_position = tic2.get_current_position()  
        

            tic.energize()
            tic.exit_safe_start()
            tic.set_target_position(int(home))
            real_position2 = tic.get_current_position()  

    
            if int(home) == real_position or int(home) == real_position2:
              print("New position is: {}.".format(int(home)))
              break
    
  
            else:
              continue
        
      

        elif(des == 'd'):
          position = tic2.get_current_position()
          position2 = tic.get_current_position()
          dump = 4000
 

          print("Current position is: {}.".format(position))
          print("Current position is: {}.".format(position2))
      
  
          while True:
            tic2.energize()
            tic2.exit_safe_start()
            tic2.set_target_position(int(dump))
            real_position = tic2.get_current_position()  

            tic.energize()
            tic.exit_safe_start()
            tic.set_target_position(int(dump))
            real_position2 = tic.get_current_position() 

    
            if int(dump) == real_position or int(dump) == real_position2:
              print("New position is: {}.".format(int(dump)))
              break
    
    
            else:
              continue
        
      
        elif(des == 'm'):
          position = tic2.get_current_position()
          position = tic.get_current_position()

          mine = 100
 

          print("Current position is: {}.".format(position))
      
  
          while True:
            tic2.energize()
            tic2.exit_safe_start()
            tic2.set_target_position(int(mine))
            real_position = tic2.get_current_position()

            tic.energize()
            tic.exit_safe_start()
            tic.set_target_position(int(mine))
            real_position2 = tic.get_current_position() 
    
            if int(mine) == real_position or int(mine) == real_position2:
              print("New position is: {}.".format(int(mine)))
              break
    
    
            else:
              continue
          
        elif(des == 'q'):
          return 0
        
      
      
        else:
          print("Error!")

      if(option =='c'):
        while True:
     
          userInput = input("Enter (w) for up, (s) for down, (q) for exit: ")

          print(tic2.get_current_position())
          print(tic.get_current_position())

          user_input = 50
      
          if (userInput == 'w'):
        
              x = tic2.get_current_position()
              b = x + 100
              tic2.set_target_position(b)
              tic.set_target_position(b)
 	
          elif(userInput == 's'):
         
             x = tic2.get_current_position()
             b = x - 100
             tic2.set_target_position(b)
             tic.set_target_position(b)
      
          elif(userInput == 'q'):
            return 0

     
          else:
             print("Error!")  
     

main()
